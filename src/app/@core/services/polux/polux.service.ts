...
export class PoluxService {
  private readonly baseUrl = (environment.API_BASES?.polux || '').replace(/\/?$/, '/');
  ...
  put<T>(resource: string, id: string | number, body: unknown): Observable<T> {
    const url = `${this.composeUrl(resource)}/${encodeURIComponent(String(id))}`;
  ...
  private composeUrl(resource: string): string {
    ...
    return `${this.baseUrl}${sanitizedResource}`;
  }
